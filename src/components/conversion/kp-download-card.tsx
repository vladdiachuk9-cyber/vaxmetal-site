...
interface KpDownloadCardProps {
  /** Defaults to the serial-production proposal shown site-wide. */
  href?: string;
  title?: string;
  subtitle?: string;
  cta?: string;
...
export function KpDownloadCard({ href, title, subtitle, cta }: KpDownloadCardProps) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-border bg-white p-6 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="font-heading text-base font-semibold text-ink">{title ?? t("title")}</h3>
          <p className="mt-1 text-sm text-steel">{subtitle ?? t("subtitle")}</p>
        </div>
      </div>
      <a
        href={href ?? "/downloads/VAXMetal_KP_serial.pdf"}
        download
